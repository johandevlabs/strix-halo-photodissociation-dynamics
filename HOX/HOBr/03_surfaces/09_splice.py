#!/usr/bin/env python3
"""
HOBr step 9: splice raster + outer shell + asymptote into one a 3A" surface.

HOCl's 15_splice.py with HOBr's pieces:

  05  CCSD(T) + EOM-CCSD raster, 1.55-2.45 A -- the accurate part, and the
      only part the band can see (03: to ~2.00 A).
  07  symmetry-forced 3A" SA-CASSCF(10,6) + SC-NEVPT2 shell, 2.25-3.85 A,
      overlapping the raster on 2.25-2.45 on purpose.
  08  E(Br, 2P) + V_OH(r_OH), the asymptote, in both method families.

The method is HOCl's unchanged, including which of its printed numbers can
actually fail. Two can: whether each (r_OH, angle) column's offset is CONSTANT
across the overlap radii -- an offset absorbs a constant, not a shape -- and
whether raster and shell agree on the SLOPE at the seam, compared before
blending, since a blend spreads a mismatch out of sight. The "local residual"
figures measure the spline and the cosine blends, which are smooth by
construction, and are not evidence. HOCl: offsets constant to a median
10 meV, slopes to 0.011 eV/A at the interior seam radius.

Changes from HOCl's:
  - E(Br) is READ from 08's CSV, not taken as a command-line default copied
    from a log, which is how HOCl's 15 got E(Cl) and how a number goes stale
    when a basis changes. --e-x-cc still overrides it.
  - Seams: raster->shell 2.25-2.45 A, shell->asymptote 3.45-3.85 A, surface
    extended on the asymptote to 6.25 A (HOCl's offsets from r_eq).
  - NPZ keys are halogen-neutral (r_ox_A, r_ox_bound_A, e_x_cc_Ha), so the
    Jacobi transform and propagation can take either molecule's file.

Usage:
    python 09_splice.py 2>&1 | tee ../logs/splice.log
"""
import argparse
import os
import csv
...
def main():
    ...
    p.add_argument("--raster", default=str(DATA / "hobr_pes_raster.csv"))
    p.add_argument("--shell", default=str(DATA / "hobr_outer_shell.csv"))
    p.add_argument("--fragments", default=str(DATA / "hobr_fragments.csv"))
    p.add_argument("--e-x-cc", type=float, default=None,
                   help="override E(Br) UCCSD(T)/x2c; by default it is "
                        "read from 08's CSV")
    p.add_argument("--join-in", type=float, default=2.25,
                   help="raster->shell blend starts here")
    p.add_argument("--join-out", type=float, default=2.45,
                   help="...and ends here; the raster's last point")
    p.add_argument("--asym-in", type=float, default=3.45,
                   help="shell->asymptote blend starts here")
    p.add_argument("--asym-out", type=float, default=3.85,
                   help="...and ends at the shell's last point")
    p.add_argument("--rmax", type=float, default=6.25,
                   help="extend the surface to here on the asymptote")
    p.add_argument("--npz", default=str(DATA / "hobr_surfaces.npz"))
    p.add_argument("--png", default=str(DATA / "hobr_surfaces.png"))
    args = p.parse_args()

    ras = read_csv(args.raster)
    she = read_csv(args.shell)
    fra = read_csv(args.fragments)
    keys = ("r_obr_A", "r_oh_A", "theta_deg")
    e_x_src = "--e-x-cc (override)"
    if args.e_x_cc is None:
        e_x_src = args.fragments
        vals = {round(float(r["e_x_uccsdt_Ha"]), 8) for r in fra
                if r.get("e_x_uccsdt_Ha") not in (None, "", "nan")}
        if len(vals) != 1:
            raise SystemExit(f"expected one E(Br) in {args.fragments}, "
                             f"found {sorted(vals)}")
        args.e_x_cc = vals.pop()

    r_ras, roh, th = grid_from(ras, keys)
    r_she, roh_s, th_s = grid_from(she, keys)
    print("=" * 78)
    print('== HOX Phase 2 -- splicing the HOBr a 3A" surface')
    print("=" * 78)
    print(f"  E(Br) UCCSD(T) {args.e_x_cc:.8f} Ha, from {e_x_src}")
    print(f"  raster {len(r_ras)}x{len(roh)}x{len(th)} over r(O-Br) "
          f"{r_ras[0]}-{r_ras[-1]} A")
    ...
    asym = args.e_x_cc + v_oh                              # (nroh,)
    resid = (VT_sh_al[-1] - asym[:, None]) * HARTREE2EV * 1000
    print(f"\n  shell (aligned) at {r_she[-1]:.2f} A minus E(Br)+V_OH: "
          f"{np.nanmin(resid):.0f} to {np.nanmax(resid):.0f} meV")
    print("  (HOCl's was -59 to +67 meV at 3.6 A, the shell still on its van der "
          "Waals tail;\n   tens of meV is expected here too)")
    ...
    print(f"\n  assembled grid {V.shape} over r(O-Br) {r_out[0]}-{r_out[-1]} A")
    ...
    band = r_out <= 2.30          # 03: the band sees only to ~2.00 A
    ...
    # an array edge (2.25 starts the shell, 2.45 ends the raster), and the
    # hand-rolled version skipped exactly those points and printed nothing.
    # Spline derivatives, not np.gradient. The raster is spaced 0.05 A and
    # the shell 0.10 A, and a central difference's truncation error (h^2
    # times the third derivative / 6) differs between them: on a synthetic
    # surface with IDENTICAL slopes in both pieces, np.gradient reported a
    # 0.06 eV/A mismatch at 2.35 A, all of it truncation error. HOCl never
    # saw it because its seam sat on a nearly flat stretch of V_T; the
    # check should not rely on that. not-a-knot splines also stay
    # accurate to the grid edges.
    g_ras = CubicSpline(r_ras, VT_in, axis=0).derivative()(r_ras) * HARTREE2EV
    g_she = CubicSpline(r_she, VT_sh_al, axis=0).derivative()(r_she) * HARTREE2EV
    ...
    for rj in ov:
        ...
        print(f"    at {rj:.2f} A: raster {np.nanmean(s_ras):+.3f}, shell "
              ...
              f"{width:.2f} A blend"
              + ("   (at a grid edge: least reliable)" if edge else ""))

    os.makedirs(os.path.dirname(os.path.abspath(args.npz)), exist_ok=True)
    np.savez_compressed(args.npz, r_ox_A=r_out, r_oh_A=roh, theta_deg=th,
                        V_T_Ha=V, r_ox_bound_A=r_ras, V_S_Ha=VS,
                        asymptote_Ha=asym, offset_Ha=off_col,
                        e_x_cc_Ha=args.e_x_cc, halogen="Br")
    print(f"\n  wrote {args.npz}")
    ...
    try:
        ...
        j = int(np.argmin(abs(roh - 0.9646)))
        ...
        ax[0, 0].set_xlabel("angle / deg"); ax[0, 0].set_ylabel("r(O-Br) / A")
        ...
        ax[0, 1].set_xlabel("r(O-H) / A"); ax[0, 1].set_ylabel("r(O-Br) / A")
        ...
        ax[1, 0].set_xlabel("r(O-H) / A"); ax[1, 0].set_ylabel("r(O-Br) / A")
        ...
        ax[1, 1].axhline(ev(asym[j]), color="grey", ls=":", label="E(Br)+V_OH")
        for v in (args.join_in, args.join_out, args.asym_in, args.asym_out):
            ax[1, 1].axvline(v, color="grey", lw=0.5)
        ax[1, 1].set_xlabel("r(O-Br) / A"); ax[1, 1].set_ylabel("E / eV")
